...
import { getInstructorCounts, listRecentItems } from '../supabase/supabaseUsers';

/**
 * PUBLIC_INTERFACE
 * InstructorDashboard shows metrics for an instructor's content and activity.
 */
    ...
    (async () => {
      try {
        ...
        const [c, r] = await Promise.all([
          getInstructorCounts(uid),
          listRecentItems({ role: 'instructor', userId: uid, limit: 5 }),
        ]);
        if (!mounted) return;
        setCounts(c);
        setRecents(r);
      ...
      {!loading && !err && (
        <>
          ...
          <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', marginTop: 12 }}>
            <RecentList title="My Recent Courses" items={recents.courses} linkBase="/courses" />
            <RecentList title="My Recent Quizzes" items={recents.quizzes} linkBase="/quizzes" />
          ...
          <div style={{ marginTop: 12, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <Link to="/quizzes/new" className="btn">Create Quiz</Link>
          </div>
        </>
      )}
    </div>
